...
const steps = [
  ...
  },
  {
    ...
    desc: "Enter up to 120 characters and watch every particle snap into each word, one by one.",
  },
  {
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none">
  ...
  },
  {
    ...
    desc: "Hit Share after typing — it copies a link that replays your exact animation for anyone who opens it.",
  },
];

export default function WelcomeModal({ onGetStarted, canShow = true }: WelcomeModalProps) {
  ...
  useEffect(() => {
    if (!canShow) return;
    const mountTimer = setTimeout(() => setMounted(true), 600);
    const fadeTimer  = setTimeout(() => setVisible(true),  680);
        ...
        <div className="px-8 pt-10 pb-8 space-y-7">
          <div className="space-y-3">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full border border-white/10 bg-white/5">
              <span className="w-1.5 h-1.5 rounded-full animate-pulse" style={{ background: "linear-gradient(135deg,#8b5cf6,#06b6d4)" }} />
              <span className="text-white/40 text-[10px] font-medium tracking-widest uppercase">Particle Flow</span>
            </div>
          ...
          <div className="space-y-2.5">
            <p className="text-white/20 text-[9px] font-medium tracking-[0.2em] uppercase">How it works</p>
              ...
              {steps.map((step, i) => (
                <button
                  ...
                  className={`w-full flex items-start gap-4 px-4 py-3.5 rounded-2xl border text-left transition-all duration-300 ${
                    activeStep === i ? "border-white/15 bg-white/6" : "border-transparent bg-transparent hover:bg-white/3"
                  }`}
                >
                  <div
                    ...
                    style={
                      activeStep === i
                        ? { background: "linear-gradient(135deg, #8b5cf620, #06b6d420)", boxShadow: "0 0 16px #8b5cf625", border: "1px solid rgba(139,92,246,0.25)" }
                  ...
                  <div className="min-w-0">
                    <p className={`text-sm font-semibold mb-0.5 transition-colors duration-300 ${activeStep === i ? "text-white" : "text-white/35"}`}>
                      {step.title}
                    </p>
                    <p className={`text-xs leading-relaxed transition-colors duration-300 ${activeStep === i ? "text-white/50" : "text-white/20"}`}>
                      {step.desc}
                    </p>
                  </div>

                  {activeStep === i && (
                    <div className="shrink-0 mt-1.5 w-1 h-1 rounded-full" style={{ background: "linear-gradient(135deg,#8b5cf6,#06b6d4)" }} />
                  )}
                </button>
              ))}
            </div>
            ...
            <div className="flex justify-center gap-1.5 pt-1">
              {steps.map((_, i) => (
                <button
                  key={i}
                  onClick={() => setActiveStep(i)}
                  className={`rounded-full transition-all duration-300 ${activeStep === i ? "w-4 h-1" : "w-1 h-1 bg-white/20"}`}
                  style={activeStep === i ? { background: "linear-gradient(90deg,#8b5cf6,#06b6d4)" } : {}}
                />
              ))}
            </div>
          </div>
          ...
          <button
            ...
            style={{ background: "linear-gradient(135deg, #8b5cf6, #06b6d4)", boxShadow: "0 4px 24px rgba(139,92,246,0.4)" }}
            onMouseEnter={(e) => { (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 0 36px rgba(139,92,246,0.65)"; }}
            onMouseLeave={(e) => { (e.currentTarget as HTMLButtonElement).style.boxShadow = "0 4px 24px rgba(139,92,246,0.4)"; }}
          >
